...
import teabagml.datasets.Arity;
import teabagml.datasets.CartesianProduct;
...
public class MapConProbTable implements ConditionalProbTable {
    ...
    private Arity arity;
    private int[] allAttributes; // the first element is the current attribute index
    private int probNum;
	...
	for (int[] eachCondition : cartProd) {
	    String conditionStr = "P(a_" + String.valueOf(allAttributes[0]) + "=" + String.valueOf(eachCondition[0]);
	    boolean firstVar = true; // used for generating '|' in the condition
	    for (int i=1; i<eachCondition.length; i++) {
		conditionStr = conditionStr + (firstVar ? " | a_" : ", a_") + String.valueOf(allAttributes[i]) + "=" + String.valueOf(eachCondition[i]);
		firstVar = false;
	    }
	    conditionStr = conditionStr + ") = " + String.valueOf(conProbTable.get(convertCondition(eachCondition)));
