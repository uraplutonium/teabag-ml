...
class ContingencyTable(object):
    ...
    def __init__(self, attributeList, adTree):
        ...
        for index in range(self.__dimension-1, 0, -1):
            self.__productList[index-1] = self.__productList[index] * arityList[self.__attributeList[index]-1]
        ctLength = 1 if not (self.__productList or self.__attributeList) else self.__productList[0]*arityList[self.__attributeList[0]-1]
        ...
        while stack:
            ...
            if ADN == -1:   # make MCV
                MCV = stack.pop()
                sliceNumber = arityList[attributeList[attributeIndex-1]-1]
                ...
                if attributeIndex<self.__dimension:   # AD-node that has Vary node children
                    ...
                    stack.append(MCV)
                    stack.append(-1)
                    for attributeValue in range(arityList[attributeList[attributeIndex]-1], 0, -1):
                        if attributeValue != MCV:
                ...
                nodesNumber = 1
                for eachArity in [arityList[attributeNum-1] for attributeNum in attributeList[attributeIndex:]]:
