...
public class MDLScoreFunction implements BNScoreFunction {

    @Override
    public double getScore(Queryable dataset, BayesNet bayesNet) {
	// System.out.println("calculating MDL score for bayesnet \"" + bayesNet.getName() + "\" on the dataset with " + dataset.getDimension() + " attributes...");
	...
	for (int eachNode=0; eachNode<dimension; eachNode++) {
	    ...
	    penalisation = penalisation + m1Queries.size()*(arity.values(eachNode)-1)/2*Math.log(dataset.getDataSize());
	    penalisation = penalisation + Math.log(dataset.getDimension())*(1+bayesNet.getParentList(eachNode).length);
	}

	// System.out.println("MDL score calculated.");
	...
	// System.out.println("MDL: " + (bicScore - penalisation));
